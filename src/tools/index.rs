...
use crate::services::cache::index_cache::get_index_cache;
...
use crate::symbols::index_symbol::match_symbol_from_query;
...
pub struct IndexDataTool;

#[async_trait]
impl Tool for IndexDataTool {
    fn name(&self) -> &str {
        "get_indices_data"
    }

    fn description(&self) -> &str {
        "Get stock index snapshot (price, change, ranges) with cache, BrightData direct API and proxy fallback. Source: Yahoo Finance https://finance.yahoo.com/quote/^{INDEX_CODE}/ (e.g., ^NSEI)."
    }
    ...
    async fn execute_internal(&self, parameters: Value) -> Result<ToolResult, BrightDataError> {
        ...
        let execution_id = format!("index_{}", chrono::Utc::now().format("%Y%m%d_%H%M%S%.3f"));
        ...
        match self.check_cache_first(query, session_id).await {
            Ok(Some(cached_result)) => {
                ...
                let formatted_response = self.create_formatted_index_response(
        ...
        match self.fetch_index_data_with_fallbacks_and_priority(
...
impl IndexDataTool {
    ...
    fn create_formatted_index_response(
        &self,
        query: &str,
        market: &str, 
        content: &str,
    ...
    /// TODO: Extract essential index data using existing filter methods
    fn extract_essential_index_data(&self, content: &str, query: &str) -> String {
        // TODO: Add essential index data extraction logic
    ...
    async fn check_cache_first(
        ...
        let cache_service = get_index_cache().await?;
        cache_service.get_cached_index_data(session_id, query).await
    }
    ...
    async fn store_in_cache(
        ...
        let cache_service = get_index_cache().await?;
        cache_service.cache_index_data(session_id, query, data.clone()).await
    }
    ...
    pub async fn get_session_cached_symbols(&self, session_id: &str) -> Result<Vec<String>, BrightDataError> {
        let cache_service = get_index_cache().await?;
        cache_service.get_session_index_symbols(session_id).await
    }
    ...
    fn build_prioritized_urls_with_priority(
        ...
        if self.is_likely_index_symbol(&clean_query) {
            match market {
                "usd" => {
    ...
    async fn fetch_index_data_with_fallbacks_and_priority(
        &self, 
        query: &str, 
        market: &str, 
        data_type: &str,
            ...
            for (url_sequence, (url, source_name)) in urls_for_method.iter().enumerate() {
                ...
                match attempt_result {
                    Ok(mut result) => {
                        ...
                        info!("✅ Successfully fetched index data from {} via {} (method: {}, url: {})", 
    ...
    fn is_likely_index_symbol(&self, query: &str) -> bool {
