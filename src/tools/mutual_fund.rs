...
use crate::services::cache::mutual_fund_cache::get_mutual_fund_cache;
...
use crate::symbols::mutual_fund_symbol::match_symbol_from_query;
...
impl Tool for MutualFundDataTool {
    ...
    fn description(&self) -> &str {
        "Get mutual fund snapshot (price/NAV, summary) with cache, BrightData direct API and proxy fallback. Source: Yahoo Finance https://finance.yahoo.com/quote/{ISIN}.BO/ (e.g., INF846K01122.BO)."
    }
        ...
        json!({
            ...
            "properties": {
                ...
                "market": {
                    ...
                    "enum": ["india", "global"],
                    "default": "india",
                    "description": "Region context (mostly 'india' for these ISIN.BO pages)"
                },
                "data_type": {
                    "type": "string",
                    "enum": ["price", "fundamentals", "news", "all"],
                    "default": "all",
                    "description": "Which slice to emphasize"
                },
                "timeframe": {
                    "type": "string",
    ...
    async fn execute_internal(&self, parameters: Value) -> Result<ToolResult, BrightDataError> {
        ...
        match self.check_cache_first(query, session_id).await {
            Ok(Some(cached_result)) => {
                ...
                let formatted_response = self.create_formatted_mutual_fund_response(
        ...
        match self.fetch_mutual_fund_data_with_fallbacks_and_priority(
            ...
            Err(_e) => {
                ...
                Ok(ToolResult::success_with_raw(
                    vec![McpContent::text("📈 **No Data Available**\n\nPlease try again with a more specific mutual_fund symbol.".to_string())],
                    empty_response
                ))
            }
        }
...
impl MutualFundDataTool {
    ...
    fn create_formatted_mutual_fund_response(
        &self,
        query: &str,
        market: &str, 
        content: &str,
    ...
    /// TODO: Extract essential mutual_fund data using existing filter methods
    fn extract_essential_mutual_fund_data(&self, content: &str, query: &str) -> String {
        // TODO: Add essential mutual_fund data extraction logic
    ...
    async fn check_cache_first(
        ...
        let cache_service = get_mutual_fund_cache().await?;
        cache_service.get_cached_mutual_fund_data(session_id, query).await
    }
    ...
    async fn store_in_cache(
        ...
        let cache_service = get_mutual_fund_cache().await?;
        cache_service.cache_mutual_fund_data(session_id, query, data.clone()).await
    }
    ...
    pub async fn get_session_cached_symbols(&self, session_id: &str) -> Result<Vec<String>, BrightDataError> {
        let cache_service = get_mutual_fund_cache().await?;
        cache_service.get_session_mutual_fund_symbols(session_id).await
    }
    ...
    pub async fn test_connectivity_with_cache(&self) -> Result<String, BrightDataError> {
        ...
        match get_mutual_fund_cache().await {
            Ok(cache_service) => {
    ...
    fn build_prioritized_urls_with_priority(
        ...
        if self.is_likely_mutual_fund_symbol(&clean_query) {
            match market {
                "usd" => {
    ...
    async fn fetch_mutual_fund_data_with_fallbacks_and_priority(
        &self, 
        query: &str, 
        market: &str, 
        data_type: &str,
            ...
            for (url_sequence, (url, source_name)) in urls_for_method.iter().enumerate() {
                ...
                match attempt_result {
                    Ok(mut result) => {
                        ...
                        info!("✅ Successfully fetched mutual_fund data from {} via {} (method: {}, url: {})", 
    ...
    fn is_likely_mutual_fund_symbol(&self, query: &str) -> bool {
