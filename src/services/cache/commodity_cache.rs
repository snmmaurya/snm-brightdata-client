// crates/snm-brightdata-client/src/services/commodity_cache.rs
...
impl CommodityCache {
    ...
    pub async fn new() -> Result<Self, BrightDataError> {
        ...
        let ttl = env::var("COMMODITY_CACHE_TTL_SECONDS")
    ...
    fn generate_key(&self, session_id: &str, symbol: &str) -> String {
        let key = format!("commodity:{}:{}", session_id, symbol.to_uppercase());
        info!("Cache Key: {}", key);
        key
    }

    /// Check if cached ETF data exists and is valid
    pub async fn get_cached_commodity_data(
        &self, 
        session_id: &str,
        symbol: &str
        ...
        match conn.get::<_, Option<String>>(&key).await {
            Ok(Some(cached_json)) => {
                match serde_json::from_str::<Value>(&cached_json) {
                    Ok(cached_data) => {
                        if let Some(timestamp) = cached_data.get("commodity_cached_at").and_then(|t| t.as_u64()) {
                            ...
                            let available_fields: Vec<String> = cached_data.as_object()
                                ...
                                .unwrap_or_default();
                            warn!("Missing 'commodity_cached_at' field for key: {} (available: {:?})", key, available_fields);
    ...
    pub async fn cache_commodity_data(
        &self,
        session_id: &str,
        symbol: &str,
        data: Value,
        ...
        cached_data["commodity_cached_at"] = json!(SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs());
        cached_data["commodity_cache_key"] = json!(key.clone());
        cached_data["commodity_cache_ttl"] = json!(self.ttl);
    ...
    pub async fn get_session_commodity_symbols(&self, session_id: &str) -> Result<Vec<String>, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
        
        // Get all keys for this session's ETFs
        let pattern = format!("commodity:{}:*", session_id);
        let keys: Vec<String> = conn.keys(&pattern).await
            .map_err(|e| BrightDataError::ToolError(format!("Redis keys command failed: {}", e)))?;
        
        // Extract symbols from keys (commodity:session:SYMBOL -> SYMBOL)
    ...
    pub async fn clear_commodity_symbol_cache(
        &self,
        session_id: &str,
        symbol: &str,
    ) -> Result<(), BrightDataError> {
    ...
    pub async fn clear_session_commodity_cache(&self, session_id: &str) -> Result<u32, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
    ...
    pub async fn get_commodity_cache_stats(&self) -> Result<Value, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
        
        // Get ETF cache key count
        let commodity_keys: Vec<String> = conn.keys("commodity:*").await.unwrap_or_default();
        ...
        let stats = json!({
            "total_commodity_cache_entries": commodity_keys.len(),
            "active_sessions": session_counts.len(),
            "session_breakdown": session_counts,
            "commodity_cache_ttl_seconds": self.ttl,
    ...
    pub async fn is_commodity_cached(&self, session_id: &str, symbol: &str) -> Result<bool, BrightDataError> {
        let key = self.generate_key(session_id, symbol);
    ...
    pub async fn batch_cache_commodity_data(
        &self,
        session_id: &str,
        commodity_data: Vec<(String, Value)>, // Vec<(symbol, data)>
    ) -> Result<Vec<String>, BrightDataError> {
        let mut successful_symbols = Vec::new();
        
        for (symbol, data) in commodity_data {
            match self.cache_commodity_data(session_id, &symbol, data).await {
                Ok(_) => {
...
static COMMODITY_CACHE: OnceCell<Arc<CommodityCache>> = OnceCell::const_new();

/// Get global ETF cache service instance
pub async fn get_commodity_cache() -> Result<Arc<CommodityCache>, BrightDataError> {
    COMMODITY_CACHE.get_or_try_init(|| async {
        let service = CommodityCache::new().await?;
        Ok(Arc::new(service))
    }).await.map(|arc| arc.clone())
}

/// Initialize ETF cache service (call this at startup)
pub async fn init_commodity_cache() -> Result<(), BrightDataError> {
    let _service = get_commodity_cache().await?;
