// crates/snm-brightdata-client/src/services/index_cache.rs
...
impl IndexCache {
    ...
    pub async fn new() -> Result<Self, BrightDataError> {
        ...
        let ttl = env::var("INDEX_CACHE_TTL_SECONDS")
    ...
    fn generate_key(&self, session_id: &str, symbol: &str) -> String {
        let key = format!("index:{}:{}", session_id, symbol.to_uppercase());
        info!("Cache Key: {}", key);
        key
    }

    /// Check if cached ETF data exists and is valid
    pub async fn get_cached_index_data(
        &self, 
        session_id: &str,
        symbol: &str
        ...
        match conn.get::<_, Option<String>>(&key).await {
            Ok(Some(cached_json)) => {
                match serde_json::from_str::<Value>(&cached_json) {
                    Ok(cached_data) => {
                        if let Some(timestamp) = cached_data.get("index_cached_at").and_then(|t| t.as_u64()) {
                            ...
                            let available_fields: Vec<String> = cached_data.as_object()
                                ...
                                .unwrap_or_default();
                            warn!("Missing 'index_cached_at' field for key: {} (available: {:?})", key, available_fields);
    ...
    pub async fn cache_index_data(
        ...
        cached_data["index_cached_at"] = json!(SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs());
        cached_data["index_cache_key"] = json!(key.clone());
        cached_data["index_cache_ttl"] = json!(self.ttl);
    ...
    pub async fn get_session_index_symbols(&self, session_id: &str) -> Result<Vec<String>, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
        
        // Get all keys for this session's ETFs
        let pattern = format!("index:{}:*", session_id);
        let keys: Vec<String> = conn.keys(&pattern).await
            .map_err(|e| BrightDataError::ToolError(format!("Redis keys command failed: {}", e)))?;
        
        // Extract symbols from keys (index:session:SYMBOL -> SYMBOL)
    ...
    pub async fn clear_index_symbol_cache(
        &self,
        session_id: &str,
        symbol: &str,
    ) -> Result<(), BrightDataError> {
    ...
    pub async fn clear_session_index_cache(&self, session_id: &str) -> Result<u32, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
    ...
    pub async fn get_index_cache_stats(&self) -> Result<Value, BrightDataError> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| BrightDataError::ToolError(format!("Redis connection failed: {}", e)))?;
        
        // Get ETF cache key count
        let index_keys: Vec<String> = conn.keys("index:*").await.unwrap_or_default();
        ...
        let stats = json!({
            "total_index_cache_entries": index_keys.len(),
            "active_sessions": session_counts.len(),
            "session_breakdown": session_counts,
            "index_cache_ttl_seconds": self.ttl,
    ...
    pub async fn is_index_cached(&self, session_id: &str, symbol: &str) -> Result<bool, BrightDataError> {
        let key = self.generate_key(session_id, symbol);
    ...
    pub async fn batch_cache_index_data(
        &self,
        session_id: &str,
        index_data: Vec<(String, Value)>, // Vec<(symbol, data)>
    ) -> Result<Vec<String>, BrightDataError> {
        let mut successful_symbols = Vec::new();
        
        for (symbol, data) in index_data {
            match self.cache_index_data(session_id, &symbol, data).await {
                Ok(_) => {
...
static INDEX_CACHE: OnceCell<Arc<IndexCache>> = OnceCell::const_new();

/// Get global ETF cache service instance
pub async fn get_index_cache() -> Result<Arc<IndexCache>, BrightDataError> {
    INDEX_CACHE.get_or_try_init(|| async {
        let service = IndexCache::new().await?;
        Ok(Arc::new(service))
    }).await.map(|arc| arc.clone())
}

/// Initialize ETF cache service (call this at startup)
pub async fn init_index_cache() -> Result<(), BrightDataError> {
    let _service = get_index_cache().await?;
